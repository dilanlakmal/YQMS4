            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 sticky top-0">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-medium text-gray-500">Inspection Time</th>
                </tr>
              </thead>
                  ...
                  <tr key={index}>
                    ...
                    <td className="px-4 py-2">
                      {log.defectDetails.map((defect, i) => (
                        <div key={i}>
                          {defect.name}: {defect.count} ({formatTime(defect.timestamp)})
                        </div>
                      ))}
                    </td>
                    <td className="px-4 py-2">{log.status}</td>
                    <td className="px-4 py-2">{log.inspectionTime} min</td>
                  </tr>
                ))}
              </tbody>
            </table>
