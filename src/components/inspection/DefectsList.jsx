...
function DefectsList({ 
  ...
  const handleDefectChange = (index, isIncrement) => {
    ...
    const currentValue = defects[index] || 0;
  ...
  const handleCurrentDefectChange = (index, isIncrement) => {
    if (!isPlaying) return;
    ...
    handleDefectChange(index, isIncrement);
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {defectItems.map((defect, index) => (
          <div
            ...
            onClick={() => isPlaying && handleCurrentDefectChange(index, true)}
            ...
            {activeCell === index && isPlaying && (
              <div className="absolute top-2 right-2 flex space-x-1">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleCurrentDefectChange(index, false);
                  }}
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleCurrentDefectChange(index, true);
                  }}
                  className="w-6 h-6 flex items-center justify-center bg-gray-200 rounded hover:bg-gray-300"
                >
                  +
        ...
        <div key={index} className={`flex justify-between items-center p-4 bg-white rounded-lg shadow-sm ${
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              <button
                disabled={!isPlaying}
                onClick={() => handleCurrentDefectChange(index, false)}
              ...
              <button
                disabled={!isPlaying}
                onClick={() => handleCurrentDefectChange(index, true)}
                className="w-8 h-8 flex items-center justify-center bg-gray-200 rounded hover:bg-gray-300 disabled:opacity-50"
              >
                +
              </button>
